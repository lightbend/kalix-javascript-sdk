...
object ModelBuilder {
      ...
      val services = for {
        ...
        service <- serviceType match {
          ...
          case ServiceType.SERVICE_TYPE_VIEW =>
            ...
            val relevantTypes = methods.flatMap(method => Seq(method.getInputType(), method.getOutputType()))
   ...
   *   the file descriptor to extract from
   */
  private def extractValueEntityDefinition(descriptor: Descriptors.FileDescriptor): Option[ValueEntity] = {
    val rawEntity =
      descriptor.getOptions
        .getExtension(com.akkaserverless.Annotations.file)
        .getValueEntity
