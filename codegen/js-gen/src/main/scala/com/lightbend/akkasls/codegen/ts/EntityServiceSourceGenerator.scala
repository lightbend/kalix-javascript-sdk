...
object EntityServiceSourceGenerator {
  ...
  def generate(
    ...
    val entityFilename = entity.fqn.name.toLowerCase + ".ts"
    ...
    val entityTestFilename = entityFilename.replace(".ts", ".test.ts")
    val testSourcePath =
  ...
  private[codegen] def testSource(
    ...
    pretty(
      ...
      "import" <+> braces(
        " " <> entityMockType <> " "
      ) <+> "from" <+> dquotes("./testkit.ts") <> semi <> line <>
