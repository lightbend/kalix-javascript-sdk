...
object SourceGenerator extends PrettyPrinter {
  ...
  /**
    ...
    * @param indexFilename  The name of the index file e.g. index.ts
    ...
    model.services.values.flatMap {
      ...
      if (model.services.nonEmpty) {
        ...
        if (!indexPath.toFile.exists()) {
          ...
          val _ = Files.write(
            indexPath,
            indexSource(model).layout.getBytes(
              Charsets.UTF_8
            )
          )
          List(indexPath)
  ...
  private[codegen] def indexSource(
    ...
    val components: Iterable[(String, String)] = model.services.values.flatMap {
      ...
      case ModelBuilder.ViewService(fqn, _, _, transformedUpdates) if transformedUpdates.nonEmpty =>
        ...
        Some((serviceName, s"./$serviceName"))
    ...
    val componentImports = components
      .foldLeft((emptyDoc, 0)) { (acc, comps) =>
        if (acc._2 == 0) {
          (
            acc._1 <> "import" <+> braces(s" ${comps._1}Component ") <+> "from" <+> dquotes(
              comps._2
            ),
            1
          )
          ...
          (
            acc._1 <> linebreak <> "import" <+> braces(
              s" ${comps._1}Component "
            ) <+> "from" <+> dquotes(comps._2),
            acc._2 + 1
          )
        }
      }
    ...
    val addComponents = components
      .foldLeft((emptyDoc, 0)) { (acc, comps) =>
        if (acc._2 == 0) {
          (acc._1 <> "addComponent" <> parens(s"${comps._1}Component"), 1)
        } else {
          (acc._1 <> linebreak <> "addComponent" <> parens(s"${comps._1}Component"), acc._2 + 1)
        }
      }
      ._1
    ...
    pretty(
      ...
      ) <> semi <> line <> componentImports <> semi <> line <> line <>
      "const" <+> "server: AkkaServerless" <+> equal <+> "new" <+> "AkkaServerless" <> parens(
        emptyDoc
      )
      <> dot <> addComponents <> dot <> "start" <> parens(emptyDoc) <> semi <> linebreak
    )
  }
  ...
  private[ts] def lowerFirst(text: String): String =
    text.headOption match {
      case Some(c) => c.toLower.toString + text.drop(1)
      case None    => ""
    }

  private[ts] def arrowFn(args: Seq[String], body: Doc) =
    parens(ssep(args.map(text), comma)) <+> "=>" <+> braces(nest(line <> body) <> line)

  private[ts] def typeReference(fqn: FullyQualifiedName): Doc = fqn match {
  ...
  private[ts] def typeUnion(fqns: Seq[FullyQualifiedName]): Doc = fqns match {
    case Nil        => " " <> "unknown"
  ...
  private[ts] def typedef(source: Doc, name: Doc): Doc =
    "@typedef" <+> braces(" " <> source <> " ") <+> name

  private[ts] def blockComment(lines: Doc*) = "/**" <> line <> ssep(
    lines.map(" *" <+> _),
    line
  ) <> line <> " */"

  private[ts] val initialisedCodeComment: Doc =
  ...
  private[ts] val managedCodeComment: Doc =
