...
function _set_sdk_version {
  ...
  if [ -n "$("$json" -q -f "$dir/package.json" "dependencies['@kalix-io/sdk']")" ] ; then
    "$json" -q -I -f "$dir/package.json" -e "this.dependencies['@kalix-io/sdk'] = '$sdk_version'"
  fi
  if [ -n "$("$json" -q -f "$dir/package.json" "devDependencies['@kalix-io/scripts']")" ] ; then
    "$json" -q -I -f "$dir/package.json" -e "this.devDependencies['@kalix-io/scripts'] = '$sdk_version'"
  fi
