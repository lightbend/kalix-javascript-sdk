...
Copy the API definition proto file from the other service into the `proto` directory but remove all `akkaserverless.service` option blocks as well as all other Kalix annotations and the `import "kalix/annotations.proto"` from it.
