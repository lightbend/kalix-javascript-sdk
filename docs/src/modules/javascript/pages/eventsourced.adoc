...
When an event is triggered, the event handler is invoked immediately. The state is updated when the emit call returns. Immediately after, the emit the state is updated to the new value and can be used to create the reply. The event hasn’t been stored yet though. Only one command is processed at a time. The next command is not processed until an event from the previous command has been successfully been stored.
