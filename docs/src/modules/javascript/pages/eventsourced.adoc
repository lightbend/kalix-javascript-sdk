...
An event sourced entity can be created using the link:{attachmentsdir}/api/module-akkaserverless.EventSourced.html[EventSourced] class.
...
The `entityType` is used to namespace events in the journal, useful for when you share the same database between multiple entities. It defaults to `valueentity`, so it's a good idea to select one explicitly.
...
The `EventSourced` class provides a helper method called link:{attachmentsdir}/api/module-akkaserverless.EventSourced.html#lookupType[`lookupType`] to facilitate this. So before implementing anything, we'll look up these types so we can use them later.
...
To create the initial state, we set the link:{attachmentsdir}/api/module-akkaserverless.EventSourced.html#initial[`initial`] callback. This takes the id of the entity being created, and returns a new empty state, in this case, an empty shopping cart:
...
A link:{attachmentsdir}/api/module-akkaserverless.EventSourced.html#~commandHandler[command handler] is a function that takes a command, the current state, and an link:{attachmentsdir}/api/module-akkaserverless.EventSourced.EventSourcedCommandContext.html[`EventSourcedCommandContext`]. It implements a service call on the entities gRPC interface.
...
A command handler may emit an event by using the link:{attachmentsdir}/api/module-akkaserverless.EventSourced.EventSourcedCommandContext.html#emit[`emit`] method on the `EventSourcedCommandContext`.
...
Once you have your command handler and event handler functions implemented, you can now set your behavior. The link:{attachmentsdir}/api/module-akkaserverless.EventSourced.html#~behaviorCallback[behavior callback] takes the current state of the entity, and returns an object with two properties, `commandHandlers` and `eventHandlers`. The callback may return different sets of handlers according to the current state, this will be explored more <<Multiple behaviors,a little later>>, for now we'll just implement an entity with one set of handlers.

The behavior callback can be set by setting the link:{attachmentsdir}/api/module-akkaserverless.EventSourced.html#behavior[`behavior`] property on the entity:
...
If you only have a single entity, as a convenience, you can start it directly, by invoking the link:{attachmentsdir}/api/module-akkaserverless.EventSourced.html#start[`start`] method, like so:
