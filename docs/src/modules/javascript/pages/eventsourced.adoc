...
include::developing:partial$eventsourced.adoc[]
...
Now we need to define the behavior for our entity. The behavior consists of two parts, xref:reference:glossary.adoc#command_handler[_command handlers_], and xref:reference:glossary.adoc#event_handler[_event handlers_].
...
An event handler's responsibility is to update the state of the entity according to the event. Event handlers are the only place where its safe to mutate the state of the Entity at all.
