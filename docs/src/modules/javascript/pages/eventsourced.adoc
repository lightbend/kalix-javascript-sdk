...
When an event is triggered, the current state is passed to each command handler. The command handler accumulates the emitted events and applies the events to the managed state after command processing.

The Java and JavaScript SDKs are different in their handling of events and state. The JavaScript doesn't apply emitted events immediately, while the Java SDK does. The JavaScript SDK fully manages the state, passing the current state in to each command handler. It accumulates the emitted events and applies them to the managed state after command processing.
