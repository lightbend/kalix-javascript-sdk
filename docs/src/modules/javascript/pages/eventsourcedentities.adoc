...
This page documents how to implement Akka Serverless event sourced entities in JavaScript. For information on what Akka Serverless event sourced entities are, please read the general xref:concepts:eventsourced.adoc[Event sourcing] documentation first.

== Persistence types and serialization

Event sourced entities persist events and snapshots, and these need to be serialized when persisted. A snapshot is the current state of the entity, and so the entities state must be serializable. The most straight forward way to persist events and the state is to use protobufs. Akka Serverless will automatically detect if an emitted event or snapshot is a protobuf, and serialize it as such using `protobufjs`. See https://www.npmjs.com/package/protobufjs for more information on `protobufjs`. For other serialization options, including JSON, see xref:serialization.adoc[Serialization].
...
[source,protobuf]
----
include::example$domain.proto[tag=eventsourced]
----
...
An event sourced entity can be created using the link:{attachmentsdir}/api/module-akkaserverless.EventSourcedEntity.html[EventSourcedEntity] class.

[source,js]
----
include::example$test/eventsourcedentity/shoppingcart.js[tag=entity-class]
----
...
The `entityType` is used to namespace events in the journal. This is useful when you have multiple entity types (i.e. shopping carts and users) in the same service.
...
When you pass an event or snapshot to Akka Serverless to persist, it needs to know how to serialize that. Simply passing a regular object does not provide enough information to know how protobuf should serialize the objects. Hence, any event or snapshot types that you want to use, you have to lookup the protobuf type for, and then use the `create` method to create it.

The `EventSourcedEntity` class provides a helper method called link:{attachmentsdir}/api/module-akkaserverless.EventSourcedEntity.html#lookupType[`lookupType`] to facilitate this. So before implementing anything, we'll look up these types so we can use them later.

[source,js]
----
include::example$test/eventsourcedentity/shoppingcart.js[tag=lookup-type]
----
...
To create the initial state, we set the link:{attachmentsdir}/api/module-akkaserverless.EventSourcedEntity.html#initial[`initial`] callback. This takes the id of the entity being created, and returns a new empty state, in this case, an empty shopping cart:

[source,js]
----
include::example$test/eventsourcedentity/shoppingcart.js[tag=initial]
----
...
A link:{attachmentsdir}/api/module-akkaserverless.EventSourcedEntity.html#~commandHandler[command handler] is a function that takes a command, the current state, and an link:{attachmentsdir}/api/module-akkaserverless.EventSourcedEntity.EventSourcedEntityCommandContext.html[`EventSourcedEntityCommandContext`]. It implements a service call on the entities gRPC interface.
...
[source,js]
----
include::example$test/eventsourcedentity/shoppingcart.js[tag=get-cart]
----
...
A command handler may emit an event by using the link:{attachmentsdir}/api/module-akkaserverless.EventSourcedEntity.EventSourcedEntity.html#emit[`emit`] method on the `EventSourcedEntityCommandContext`.

Here's an example of a command handler that emits an event:

[source,js]
----
include::example$test/eventsourcedentity/shoppingcart.js[tag=add-item]
----

This command handler also validates the command, ensuring the quantity items added is greater than zero. Invoking link:{attachmentsdir}/api/module-akkaserverless.EventSourcedEntity.EventSourcedEntityCommandContext.html#fail[`fail`] fails the command - this method throws so there's no need to explicitly throw an exception.

=== Event handlers

An link:{attachmentsdir}/api/module-akkaserverless.EventSourcedEntity.html#~eventHandler[event handler] is invoked at two points, when restoring entities from the journal, before any commands are handled, and each time a new event is emitted. An event handlers responsibility is to update the state of the entity according to the event. Event handlers are the only place where its safe to mutate the state of the entity at all.
...
[source,js]
----
include::example$test/eventsourcedentity/shoppingcart.js[tag=item-added]
----

=== Setting the behavior

Once you have your command handler and event handler functions implemented, you can now set your behavior. The link:{attachmentsdir}/api/module-akkaserverless.EventSourcedEntity.html#~behaviorCallback[behavior callback] takes the current state of the entity, and returns an object with two properties, `commandHandlers` and `eventHandlers`. The callback may return different sets of handlers according to the current state, this will be explored more <<Multiple behaviors,a little later>>, for now we'll just implement an entity with one set of handlers.

The behavior callback can be set by setting the link:{attachmentsdir}/api/module-akkaserverless.EventSourcedEntity.html#behavior[`behavior`] property on the entity:

[source,js]
----
include::example$test/eventsourcedentity/shoppingcart.js[tag=behavior]
----
...
[source,js]
----
include::example$test/eventsourcedentity/shoppingcart.js[tag=multiple-behaviors]
----

== Starting the entity

If you only have a single entity, as a convenience, you can start it directly, by invoking the link:{attachmentsdir}/api/module-akkaserverless.EventSourcedEntity.html#start[`start`] method, like so:

[source,js]
----
include::example$test/eventsourcedentity/shoppingcart.js[tag=start]
----
...
[source,js]
----
include::example$test/eventsourcedentity/shoppingcart.js[tag=add-component]
----
