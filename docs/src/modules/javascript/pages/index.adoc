...
To save the work of starting from scratch, the Java xref:java:kickstart.adoc[code generation tool] creates a project, complete with descriptors and implementations. Or, you can start from one of our xref:tutorial:index.adoc[example applications].
...
This page documents how to implement Replicated Entities in Javascript. For information on what  Replicated Entities are, please read how to xref:designing:choose-state-model.adoc[choose a state model] and the general xref:developing:crdts.adoc[Replicated Entity] information first.
