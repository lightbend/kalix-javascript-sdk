...
* The xref:getting-started:set-up-development-env.adoc#_akka_serverless_cli[`akkasls` CLI downloaded]
* A configured Docker Registry
* An xref:getting-started:lightbend-account.adoc[Akka Serverless account]
* An xref:getting-started:projects.adoc[Akka Serverless deployment project]

:sectnums:
== Prepare the development project

The code generation tools create a new development project named `my-entity`, that contains build configuration and the xref:reference:glossary.adoc#gRPC[gRPC Protobuf specifications] for a xref:reference:glossary.adoc#value_entity[Value Entity].
...
To understand how the Protobuf definitions relate to the implementation files, see xref:designing:working.adoc#dev-process[The development process] and xref:designing:programming-model.adoc[].
...
You can run your services locally and test them manually with HTTP or gRPC requests. To run locally, you need to initiate the Akka Serverless xref:reference:glossary.adoc#proxy[proxy]. The `docker-compose` file contains the configuration required to run the proxy.
...
+
Refer to xref:deploying:registries.adoc[] for more information on how to make your docker image available to Akka Serverless.
