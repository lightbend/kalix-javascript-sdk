...
Note that Value Entities and Event Sourced Entities are slightly different based upon codegen requirements. Other than the code generation annotations, the proto file are materially equivalent for the same use-case. See the following examples which show the difference.
