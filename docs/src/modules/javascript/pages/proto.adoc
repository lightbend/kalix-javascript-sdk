...
To use a multi-field key, add the `entity_key` notation to each field. For example, if the domain proto file `AddLineItem` element defines both the `user_id` and `product_id` as part of the key, do the same for the service message:

[source,proto]
----
message AddLineItem {
    string user_id = 1 [(akkaserverless.field).entity_key = true];
    string product_id = 2 [(akkaserverless.field).entity_key = true];
----
...
To use a multi-field key, add the `entity_key` notation to each field. For example, if the service API proto file `AddLineItem` element defines both the `user_id` and `product_id` as part of the key, do the same for the service message:
