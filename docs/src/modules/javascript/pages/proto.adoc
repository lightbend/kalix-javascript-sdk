...
It will often be necessary to access the generated entity key from inside the entities code. This can be done using the link:{attachmentsdir}/api/module-kalix.EntityContext.html#entityId[entityId{tab-icon}, window="new"] property on the passed in context.
