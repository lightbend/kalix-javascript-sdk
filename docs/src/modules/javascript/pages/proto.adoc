...
To use a multi-field key, add the `entity_key` notation to each field. For example, if the service API proto file `SomeMessage` element defines both `first_field` and `second_field` as part of the key, do the same for the domain message:
