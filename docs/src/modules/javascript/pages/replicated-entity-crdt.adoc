...
A Replicated Entity is implemented with the link:{attachmentsdir}/api/module-kalix.replicatedentity.ReplicatedEntity.html[ReplicatedEntity{tab-icon}, window="new"] class:
...
We need to implement all methods our Replicated Entity offers as https://docs.kalix.io/reference/glossary.html#command_handler[_command handlers_]. A link:{attachmentsdir}/api/module-kalix.replicatedentity.ReplicatedEntity.html#~commandHandler[`command handler`{tab-icon}, window="new"] is a function that takes a command and a link:{attachmentsdir}/api/module-kalix.replicatedentity.ReplicatedEntityCommandContext.html[`ReplicatedEntityCommandContext`{tab-icon}, window="new"]. The command is the input message type for the gRPC service call, and the command handler must return a message of the same type as the output type of the gRPC service call.

We map each method to a command handler function using link:{attachmentsdir}/api/module-kalix.replicatedentity.ReplicatedEntity.html#commandHandlers[`commandHandlers`{tab-icon}, window="new"].
...
The current Replicated Data value can be accessed using the link:{attachmentsdir}/api/module-kalix.replicatedentity.ReplicatedEntityCommandContext.html#state[`state`{tab-icon}, window="new"] method on the provided context.
...
The following example shows the implementation of the `RemoveCart` command handler. Replicated Entity instances for a particular entity identifier can be deleted, using the link:{attachmentsdir}/api/module-kalix.replicatedentity.ReplicatedEntityCommandContext.html#delete[`delete`{tab-icon}, window="new"] method on the provided context. Once deleted, an entity instance cannot be recreated, and all subsequent commands for that entity identifier will be rejected with an error.
...
A link:{attachmentsdir}/api/module-kalix.replicatedentity.ReplicatedCounter.html[ReplicatedCounter{tab-icon}, window="new"] can be incremented and decremented.
...
A link:{attachmentsdir}/api/module-kalix.replicatedentity.ReplicatedRegister.html[ReplicatedRegister{tab-icon}, window="new"] can contain any (serializable) value. Updates to the value are replicated using last-write-wins semantics, where concurrent modifications are resolved by using the update with the highest timestamp.
...
A link:{attachmentsdir}/api/module-kalix.replicatedentity.ReplicatedSet.html[ReplicatedSet{tab-icon}, window="new"] is a set of (serializable) values, where elements can be added or removed.
...
A link:{attachmentsdir}/api/module-kalix.replicatedentity.ReplicatedCounterMap.html[ReplicatedCounterMap{tab-icon}, window="new"] maps (serializable) keys to replicated counters, where each value can be incremented and decremented.
...
A link:{attachmentsdir}/api/module-kalix.replicatedentity.ReplicatedRegisterMap.html[ReplicatedRegisterMap{tab-icon}, window="new"] maps (serializable) keys to replicated registers of (serializable) values. Updates to values are replicated using last-write-wins semantics, where concurrent modifications are resolved by using the update with the highest timestamp.
...
A link:{attachmentsdir}/api/module-kalix.replicatedentity.ReplicatedMultiMap.html[ReplicatedMultiMap{tab-icon}, window="new"] maps (serializable) keys to replicated sets of (serializable) values, providing a multi-map interface that can associate multiple values with each key.
...
A link:{attachmentsdir}/api/module-kalix.replicatedentity.ReplicatedMap.html[ReplicatedMap{tab-icon}, window="new"] maps (serializable) keys to any other Replicated Data types, allowing a heterogeneous map where values can be of any Replicated Data type.
