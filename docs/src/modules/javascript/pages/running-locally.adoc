...
[TIP]
====
The samples provided with the Kalix SDKs all have `docker-compose` files preconfigured to run your service locally. They also contain the configuration to start a local Google Pub/Sub emulator that the Kalix Runtime will connect to when you make use of topic-based Eventing in your service.
====

== Starting your service

To start the Kalix Runtime, run the following command from the directory containing the `docker-compose.yml` file:
...
<2> The address of the Kalix Runtime (using `plaintext` instead of TLS)
<3> The RPC procedure name which is deduced from the protobuf definition of the component you're calling.


== Shut down the Runtime

To shut down the Kalix Runtime, which removes all the data kept in memory, run the command below:

[source, command line]
----
docker stop kalix-runtime
----

To delete the Kalix Runtime container, run

[source, command line]
----
docker rm kalix-runtime
----
...
. Each service will need its own Kalix Runtime container and each Runtime container will need its own port.

See below how to do such configuration.

=== Run user function on a custom port

When using the defaults, the Kalix Runtime connects to the user function on port 8080. However, if you're looking to have 2 services running at the same time, the second service will need to be run at a different port (e.g. 8081).
...
=== Run the Kalix Runtime on a custom port

To update the docker compose file to run multiple Kalix Runtime instances, add the snippet below:
...
<2> The host port ("900x") in the port mapping must be unique for each Runtime instance. Note that the internal Runtime port should also be changed hence the extra `-Dkalix.proxy.http-port=900x` option.
