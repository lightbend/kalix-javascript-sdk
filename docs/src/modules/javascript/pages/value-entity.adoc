...
<3> The protobuf option `(akkaserverless.file).value_entity` is specific to code-generation as provided by the Kalix plugin.
...
<7> The protobuf option `(akkaserverless.service)` is specific to code-generation as provided by Kalix and points to the protobuf definition `Counter` shown above (in `com.example.domain`).
...
* The protobuf files, `counter_api.proto` and `counter_domain.proto`, define the service and the domain protocol. Kalix uses the service protobuf file to load and read the service. It uses the definitions in `counter_domain.proto` to serialize state it receives from the https://developer.lightbend.com/docs/akka-serverless/reference/glossary.html#proxy[proxy].
