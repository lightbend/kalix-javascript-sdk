...
<3> The protobuf option `(kalix.file).value_entity` is specific to code-generation as provided by the Kalix plugin.
...
<7> The `CounterState` protobuf message is what Kalix stores for this entity.
...
<7> The protobuf option `(kalix.service)` is specific to code-generation as provided by Kalix and points to the protobuf definition `Counter` shown above (in `com.example.domain`).

== Creating an Entity

The following code creates the Value Entity with the link:{attachmentsdir}/api/module-kalix.ValueEntity.html[`ValueEntity`{tab-icon}, window="new"] class. It passes in:
...
Use the `ValueEntity` link:{attachmentsdir}/api/module-kalix.ValueEntity.html#lookupType[`lookupType`{tab-icon}, window="new"] helper to look up these types so we can use them later.
...
To create the initial state, set the link:{attachmentsdir}/api/module-kalix.ValueEntity.html#initial[`initial`{tab-icon}, window="new"] callback. This takes the ID of the entity being created, and returns a new empty state, in this case, an empty counter state:
...
Value Entity behavior is defined in command handlers. A link:{attachmentsdir}/api/module-kalix.ValueEntity.html#~commandHandler[command handler{tab-icon}, window="new"] is a function that takes a command, the current state, and a link:{attachmentsdir}/api/module-kalix.ValueEntity.ValueEntityCommandContext.html[`ValueEntityCommandContext`{tab-icon}, window="new"]. The function executes a service call on the Entity gRPC interface.
...
When updating the state, a command handler MUST persist that change by calling the link:{attachmentsdir}/api/module-kalix.ValueEntity.ValueEntityCommandContext.html#updateState[`updateState`{tab-icon}, window="new"] method on the `ValueEntityCommandContext`. If it does not, any change to the state will be *lost* when the next command arrives.

The following command handler updates the state. It also validates the command, ensuring the quantity of items added is greater than zero. Invoking link:{attachmentsdir}/api/module-kalix.ValueEntity.ValueEntityCommandContext.html#fail[`fail`{tab-icon}, window="new"] fails the command. This method throws, so there's no need to explicitly throw an exception.
