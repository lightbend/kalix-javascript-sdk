...
The `entityType` is used to namespace the state in the journal, useful for when you share the same database between multiple entities. It defaults to `entity`, so it's a good idea to select one explicitly.
...
[source,js]
----
include::example$test/valueentity/shoppingcart.js[tag=add-component]
----
