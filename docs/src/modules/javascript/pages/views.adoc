...
Consider an example of a Customer Registry service with a `customer` Value Entity. When `customer` state changes, the entire state is emitted as a value change. Value changes update any associated Views. To create a View that shows customers by their name:
...
* A `table` property that you will use in the query.
...
<3> The `(akkaserverless.method).view.update` annotation defines that this method is used for updating the View. You must define the `table` property to provide a name for the table to be used in the query. You can use any name.
...
To use the View, invoke the `addComponent` function. For example:
...
The example assumes a `customer_domain.proto` file defines events that will update the View because a name changed:
...
* Specifies a `table` attribute used by Akka Serverless to store the View. You can use any name.
...
<5> Same `event_sourced_entity` for all update methods. Note the `table` attribute that can be any name.
...
In the client, use the `addComponent` function to register the View. For example:
...
Define View queries in a language that is similar to SQL. The following examples illustrate the syntax for a `customers` entity. To retrieve:
