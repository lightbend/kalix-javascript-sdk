...
describe("Customer registry service", function() {
  ...
  it("should get existing customers", async () => {
    expect(await client().getCustomerAsync({customerId: "alice"})).to.deep.equal(alice);
    expect(await client().getCustomerAsync({customerId: "bob"})).to.deep.equal(bob);
    expect(await client().getCustomerAsync({customerId: "alice2"})).to.deep.equal(alice2);
  });

  it("should lookup customers by name", async () => {
    expect(await view().getCustomersAsync({customerName: "Alice"})).to.deep.equal({ results: [alice, alice2] });
    expect(await view().getCustomersAsync({customerName: "Bob"})).to.deep.equal({ results: [bob] });
  });
  ...
  it("should change customer names", async () => {
    ...
    expect(await view().getCustomersAsync({customerName: "Alice"})).to.deep.equal({ results: [alice] });
    expect(await view().getCustomersAsync({customerName: "Other Alice"})).to.deep.equal({ results: [otherAlice] });
  });
