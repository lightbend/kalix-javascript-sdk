...
const testkit = new IntegrationTestkit().addComponent(shoppingcart);
...
describe("Shopping cart service", function () {
  ...
  it("should remove a cart", async () => {
    ...
    // after delete
    await expect(client().getCartAsync({ cartId: "cart4" })).to.be.rejectedWith("Entity deleted");
  });
});
