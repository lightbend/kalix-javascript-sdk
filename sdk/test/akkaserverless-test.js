...
describe("Akkaserverless", () => {
  ...
  it("should generate working links based on error codes", () => {
    // Arrange
    const akkasls = new AkkaServerless({descriptorSetPath: "test/user-function-test.desc"});
    ...
    // Assert
    specificLink.should.equal("https://developer.lightbend.com/docs/akka-serverless/javascript/views.html#changing");
    componentLink.should.equal("https://developer.lightbend.com/docs/akka-serverless/javascript/views.html");
...
At package.test.json:2:4:
  ...
  it("discovery service should return correct service info", () => {
    ...
    serviceInfo.getSupportLibraryName().should.equal("@lightbend/akkaserverless-javascript-sdk");
  ...
  it("discovery service should return correct components with passivation", () => {
    ...
    result.getComponentsList().length.should.equal(1);
    const comp = result.getComponentsList()[0];
    comp.getEntity().getPassivationStrategy().getTimeout().getTimeout().should.equal(10);
  });
