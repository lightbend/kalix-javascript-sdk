...
if [ -d "$proto_dir" ] && [ -d "$generated_dir" ] && [ -f "$descriptor_file" ] ; then
  ...
  if [ -n "$PROXY_SNAPSHOT_DIRECTORY" ]; then
    ...
    artifact_url="https://s01.oss.sonatype.org/content/repositories/snapshots/com/akkaserverless/kalix-tck-protocol/$framework_version/kalix-tck-protocol-$framework_version.zip"
