...
function process(request, context) {
  if (context.state === null) context.state = createReplicatedData(context.entityId);
  request.actions.forEach(action => {
    if (action.update) {
      if (action.update.writeConsistency === ReplicatedData.WriteConsistencies.LOCAL)
        context.writeConsistency = ReplicatedData.WriteConsistencies.LOCAL
      else if (action.update.writeConsistency === ReplicatedData.WriteConsistencies.MAJORITY)
        context.writeConsistency = ReplicatedData.WriteConsistencies.MAJORITY
      else if (action.update.writeConsistency === ReplicatedData.WriteConsistencies.ALL)
...
function applyUpdate(update, state) {
  if (update.gcounter) {
    ...
    if (update.lwwregister.clock.clockType === ReplicatedData.Clocks.REVERSE)
      state.setWithClock(update.lwwregister.value, ReplicatedData.Clocks.REVERSE);
    else if (update.lwwregister.clock.clockType === ReplicatedData.Clocks.CUSTOM)
      state.setWithClock(update.lwwregister.value, ReplicatedData.Clocks.CUSTOM, update.lwwregister.clock.customClockValue);
    else if (update.lwwregister.clock.clockType === ReplicatedData.Clocks.CUSTOM_AUTO_INCREMENT)
      state.setWithClock(update.lwwregister.value, ReplicatedData.Clocks.CUSTOM_AUTO_INCREMENT, update.lwwregister.clock.customClockValue);
    else
...
function responseValue(context) {
  return Response.create(context.noState ? {} : { state: replicatedDataState(context.state) });
}
...
function replicatedDataState(state) {
  if (state instanceof ReplicatedData.GCounter)
    ...
    return { ormap: state.size ? { entries: sortedEntries(state.entries(), replicatedDataState) } : {} };
